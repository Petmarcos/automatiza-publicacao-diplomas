...
async def api_processar(file_digitais: UploadFile = File(...), file_emitidos: UploadFile = File(...)):
    try:
        ...
        # --- AJUSTE AQUI ---
        # A função agora retorna um dicionário com 'rtf' e 'html_previa'
        resultado_rtf = gerar_texto_rtf(df_resultado, resumo_livros, total_geral)
        texto_rtf = resultado_rtf["rtf"]
        html_previa = resultado_rtf["html_previa"]
        # -------------------
        
        # Converte para JSON para o React ler
        lista_previa = df_resultado.to_dict(orient="records")
        
        # Retornamos o 'html_previa' para que a tela do React renderize de forma WYSIWYG
        return {
            "total_geral": total_geral,
            "previa_texto_rtf": texto_rtf,
            "previa_html": html_previa,     # ← Nova propriedade adicionada aqui
